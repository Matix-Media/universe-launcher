<template>
    <div>
        ...
        <header-list title="Mods" v-if="loaded">
            <p class="error" v-if="error">
                The mods could not be loaded.
            </p>
            <p class="empty" v-if="mods.length == 0">
                There seems to be no mods in this modpack.
            </p>
            <div class="news-box" v-for="item in mods" :key="item.ID">
                <div class="header">
                    <h4>{{item.name}} <a v-if="item.url" :href="item.url" target="_blank" class="open-url"><i class="fas fa-external-link-alt"></i></a></h4>
                    <p class="author"><span>by</span> {{item.author}}</p>
                </div>
...
export default {
    ...
    mounted() {
        this.$nextTick(() => {
            this.$api.getMods(this.$route.params.id).then((res) => {
                console.log("Loaded!");
                this.mods = res;
                this.loaded = true;
            }).catch(() => {
...
div.news-box h4 .open-url {
    margin-left: .8rem;
    color: rgba(255, 255, 255, 0.2);
    transition: color .2s;
}

div.news-box h4 .open-url:hover {
...
div.news-box p.author {
    color: rgba(255, 255, 255, 0.20);
}

div.news-box p.author span {
